...
def process_files(file_paths):
    ...
    prompt = f"""
    Определи тип документа по входным данным.
    Этот текст из паспорта или из трудовой книжки? 
    Этот текст про одного человека.
    Трудовая книжка содержит текст "Трудовой договор"
    Трудовая книжка содержит информацию о профессии человека
    Смена мест работы - трудовая книжка
    Паспорт содержит основные данные человека, адрес, местоположение, в том числе изменения адреса проживания, семейного положения.
    Напиши только название типа документа.
    Входные данные:
